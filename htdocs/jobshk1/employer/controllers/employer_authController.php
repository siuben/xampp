...
if (isset($_POST['signup-btn'])) {
    ...
    if (empty($admin_username)) {
        $errors['admin_username'] = "Company Name required";
    }
    ...
    if (count($errors) === 0) {
        ...
        $sql = "INSERT INTO admin_login (admin_username, admin_email, verified, token, admin_pass) VALUES (?, ?, ?, ?, ?)";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param('ssbss', $admin_username, $admin_email, $verified, $token, $password); /*該函式繫結了 SQL 的引數，且告訴資料庫引數的值。 “sss” 引數列處理其餘引數的資料型別。s 字元告訴資料庫該引數為字串。*/
        if($stmt->execute()){
            // login user
...
<body>
    ...
    <a href="http://localhost/jobshk/employer/admin_dashboard.php?token=' . $token . '"> 
...
<body>
    ...
    <a href="http://localhost/jobshk/week19/index.php?password-token=' . $token . '"> 
        Reset your password
    </a>
</body>
</html>';
